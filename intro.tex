...
However, the beam momentum measured is constrained to between 13 and 160 GeV/c.
...
In order to constrain hadron production uncertainties, work is in progress for the EMPHATIC (Experiment Measuring the Production of Hadrons At a Test-beam In Chicagoland) experiment.
...
After passing through the ARICH, particles will pass through a resistive plate chamber time-of-flight (RPC ToF) detector.
...
Currently, development is underway on the simulation and design of the ARICH detector.
...
The method by which this particle identification is done is  outlined in Section \ref{sec:particleIdentification}, and requires efficient and realistic Monte Carlo simulations of particle propagation in the detector.
Furthermore, it is crucial that simulations be done to characterize the detector's performance and optimize the design.
For these reasons, the development of fast simulations of the ARICH detector is an important component of the design of the EMPHATIC experiment, which in turn is a crucial contribution to the future of neutrino investigations.
